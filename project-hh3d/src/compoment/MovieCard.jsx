...
function MovieCard({ movie }) {
  ...
  // Logic: Ưu tiên ảnh local -> nếu lỗi thì ẩn ảnh hiện nền xám
  const imageUrl = movie.image 
    ? (movie.image.startsWith('http') ? movie.image : `/image/${movie.image}`)
    : null;

  return (
    <Link to={`/movie/${movie.id}`} className="group relative block transition-all hover:-translate-y-2">
      <div className="overflow-hidden rounded-xl aspect-[2/3] border border-gray-800 shadow-lg relative bg-[#1c1c1c]">
        {imageUrl ? (
          <img 
            src={imageUrl} 
            alt={movie.title} 
            ...
            onError={(e) => {
              e.target.style.display = 'none'; // Nếu file pnt.jpg không có trong folder image, ẩn ảnh đi
            }} 
          />
        ) : null}
        
        {/* Lớp phủ khi không có ảnh hoặc ảnh lỗi */}
        <div className="absolute inset-0 flex items-center justify-center text-gray-600 text-[10px] text-center p-2 pointer-events-none">
          {!imageUrl && "Chưa có Poster"}
        </div>
